"""Baseline schema from scripts/schema.sql.

Production databases that were created from scripts/schema.sql should be
stamped at this revision once before applying newer migrations:

    alembic stamp 0001_baseline_schema
"""

from __future__ import annotations

from typing import Sequence

from alembic import op

revision: str = "0001_baseline_schema"
down_revision: str | None = None
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


def upgrade() -> None:
    op.execute(
        """
        CREATE TYPE "professional_status" AS ENUM (
          'register_pending',
          'under_review',
          'approved',
          'rejected',
          'payment_pending',
          'active',
          'inactive'
        );

        CREATE TYPE "channel" AS ENUM (
          'TELEGRAM',
          'WHATSAPP'
        );

        CREATE TABLE IF NOT EXISTS "person" (
          "id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
          "phone_number" varchar UNIQUE NOT NULL,
          "chat_state" varchar NOT NULL,
          "name" varchar,
          "cpf" varchar UNIQUE,
          "channel" channel,
          "created_at" timestamp NOT NULL
        );

        CREATE TABLE IF NOT EXISTS "professional" (
          "id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
          "person_id" int UNIQUE NOT NULL,
          "area" varchar NOT NULL,
          "professional_register" varchar NOT NULL,
          "register_type" varchar NOT NULL,
          "approach" text,
          "background" text,
          "video_platform" varchar,
          "email" varchar,
          "status_id" int UNIQUE NOT NULL,
          "created_at" timestamp NOT NULL
        );

        CREATE TABLE IF NOT EXISTS "patient" (
          "id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
          "person_id" int UNIQUE NOT NULL,
          "created_at" timestamp NOT NULL
        );

        CREATE TABLE IF NOT EXISTS "professional_patient" (
          "id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
          "professional_id" int NOT NULL,
          "patient_id" int NOT NULL,
          "created_at" timestamp NOT NULL,
          "deleted" boolean NOT NULL DEFAULT FALSE
        );

        CREATE TABLE IF NOT EXISTS "message_history" (
          "id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
          "person_id" int NOT NULL,
          "created_at" timestamp NOT NULL,
          "content" text,
          "image_url" text,
          "document_url" text,
          "is_from_user" boolean NOT NULL
        );

        CREATE TABLE IF NOT EXISTS "professional_status_history" (
          "id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
          "professional_id" int NOT NULL,
          "professional_status" professional_status NOT NULL,
          "created_at" timestamp NOT NULL
        );

        ALTER TABLE "professional"
          ADD FOREIGN KEY ("person_id")
          REFERENCES "person" ("id")
          DEFERRABLE INITIALLY IMMEDIATE;

        ALTER TABLE "professional"
          ADD FOREIGN KEY ("status_id")
          REFERENCES "professional_status_history" ("id")
          DEFERRABLE INITIALLY IMMEDIATE;

        ALTER TABLE "patient"
          ADD FOREIGN KEY ("person_id")
          REFERENCES "person" ("id")
          DEFERRABLE INITIALLY IMMEDIATE;

        ALTER TABLE "professional_patient"
          ADD FOREIGN KEY ("professional_id")
          REFERENCES "professional" ("id")
          DEFERRABLE INITIALLY IMMEDIATE;

        ALTER TABLE "professional_patient"
          ADD FOREIGN KEY ("patient_id")
          REFERENCES "patient" ("id")
          DEFERRABLE INITIALLY IMMEDIATE;

        ALTER TABLE "message_history"
          ADD FOREIGN KEY ("person_id")
          REFERENCES "person" ("id")
          DEFERRABLE INITIALLY IMMEDIATE;

        ALTER TABLE "professional_status_history"
          ADD FOREIGN KEY ("professional_id")
          REFERENCES "professional" ("id")
          DEFERRABLE INITIALLY IMMEDIATE;
        """
    )


def downgrade() -> None:
    op.execute(
        """
        DROP TABLE IF EXISTS "professional_status_history" CASCADE;
        DROP TABLE IF EXISTS "message_history" CASCADE;
        DROP TABLE IF EXISTS "professional_patient" CASCADE;
        DROP TABLE IF EXISTS "patient" CASCADE;
        DROP TABLE IF EXISTS "professional" CASCADE;
        DROP TABLE IF EXISTS "person" CASCADE;
        DROP TYPE IF EXISTS "professional_status";
        DROP TYPE IF EXISTS "channel";
        """
    )
